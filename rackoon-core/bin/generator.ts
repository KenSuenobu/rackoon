...
(async () => {
  function parseSchemaForType(schema: any) {
    ...
    if (schema["$ref"]) {
      return {
        name: schema["$ref"].substring(schema["$ref"].lastIndexOf("/") + 1),
        isClass: true,
      };
    }
  ...
  for (const tagName of Object.keys(tagFunctions)) {
      ...
      for (const def of tagFunctions[tagName][op]) {
        ...
        if (def["parameters"]) {
          for (const param of def["parameters"]) {
            ...
            parameters.push(
              `${name}${paramRequired ? "?" : ""}: ${paramType.name}`
            );
        ...
        Array.from(classes.values()).forEach((x) => {
          const className: string = x as string;

          header += `import { ${className} } from '../model/${
        ...
        for (const response of Object.keys(def["responses"])) {
          ...
          if (responseObject["content"]) {
            responseReturnType = parseSchemaForType(
    ...
    console.log(
      `- Generating Delegate for '${delegateClass}' (${numOperations} operations) -> src/api/${delegateClass}.ts`
    );
    fs.writeFileSync(
      `src/api/${delegateClass}.ts`,
      header + (header !== "" ? "\n" : "") + body
    );
  }
  ...
  console.log(
    `- Generating ${baseName} Controller module -> src/api/${baseName}Controller.ts`
  );
  ...
  Object.keys(tagFunctions).forEach((tagName) => {
    const delegateName = `${
      tagName.toLowerCase().substring(0, 1) + tagName.substring(1)
    }ApiDelegateImpl`;
    const delegateAccessorName = `${
      tagName.toLowerCase().substring(0, 1) + tagName.substring(1)
    }DelegateImpl`;

    controllerBody += `    private ${delegateName} = new ${tagName}APIDelegate();\n`;
    accessorBody +=
      `    public get ${delegateAccessorName}(): ${tagName}APIDelegate {\n` +
      `        return this.${delegateName};\n` +
      "    }\n\n";
    accessorBody +=
      `    public set ${delegateAccessorName}(impl: ${tagName}APIDelegate) {\n` +
  ...
  for (const tagName of Object.keys(tagFunctions)) {
    ...
    for (const op of Object.keys(tagFunctions[tagName])) {
      for (const def of tagFunctions[tagName][op]) {
        ...
        routeBody +=
          "                res.status(e.status ?? 500).send(`An error occurred while processing this request.<p/>\\n<pre>${e.stack}</pre>\\n`);\n";
        routeBody += "                res.end();\n";
        routeBody += "            }\n";
        routeBody += "        });\n\n";
      }
    ...
    console.log(
      `- Generating Routes for '${tagName}Router' -> src/routes/${tagName}Router.ts`
    );

    fs.writeFileSync(`src/routes/${tagName}Router.ts`, routeBody);
  }

  console.log(`- Generating ${baseName}Router.ts (Master Router Class)`);
